...
/* The base register these tests use. Not RN: arm.h defines that as the field
   extractor for the register number in an opcode, and this header is included
   above, so the two collide. */
#define TEST_RN 12
...
static void setregs(uint32_t base){ for(int r=0;r<15;r++) arm.reg[r]=0x1000+r*0x111; arm.reg[TEST_RN]=base; }
static uint32_t op(int P,int U,int W,int L,uint32_t list){ return (0xeu<<28)|(0x4u<<25)|((uint32_t)P<<24)|((uint32_t)U<<23)|((uint32_t)W<<21)|((uint32_t)L<<20)|((uint32_t)TEST_RN<<16)|(list&0xffff); }
static void chk(const char*nm,uint32_t opcode,uint32_t base){
    ...
    for(int r=0;r<16;r++) ireg[r]=arm.reg[r];
    memcpy(imem,membuf,4096);
    ...
    if(bad){ if(failures<25){ fprintf(stderr,"MISMATCH %s op=%08x base=%08x mem=%s",nm,opcode,base,memcmp(imem,membuf,4096)?"DIFF":"ok");
        for(int r=0;r<15;r++) if(ireg[r]!=jreg[r]) fprintf(stderr," r%d(i=%08x j=%08x)",r,ireg[r],jreg[r]);
        fprintf(stderr,"\n"); } failures++; }
}
